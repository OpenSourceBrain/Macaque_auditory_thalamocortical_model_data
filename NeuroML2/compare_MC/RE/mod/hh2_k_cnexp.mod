TITLE Mod file for component: Component(id=hh2_k type=ionChannelHH)
...
NEURON {
    ...
    RANGE gion
    RANGE i__hh2_k : a copy of the variable for current which makes it easier to access from outside the mod file
...
PARAMETER {
    ...
    n_forwardRate_rate = 0.16000001 (kHz)  : was: 160.0 (per_time)
    n_forwardRate_midpoint = -35 (mV)      : was: -0.035 (voltage)
    n_forwardRate_scale = 5 (mV)           : was: 0.005 (voltage)
    n_reverseRate_rate = 0.5 (kHz)         : was: 500.0 (per_time)
    n_reverseRate_midpoint = -40 (mV)      : was: -0.04 (voltage)
    n_reverseRate_scale = -40 (mV)         : was: -0.04 (voltage)
    n_q10Settings_q10Factor = 3            : was: 3.0 (none)
    n_q10Settings_experimentalTemp = 309.15 (K): was: 309.15 (temperature)
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=hh2_k type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=hh2_k type=ionChannelHH), from gates; Component(id=n type=gateHHrates)
    ...
    ik = gion * (v - ek)
    i__hh2_k =  -1 * ik : set this variable to the current also - note -1 as channel current convention for LEMS used!
