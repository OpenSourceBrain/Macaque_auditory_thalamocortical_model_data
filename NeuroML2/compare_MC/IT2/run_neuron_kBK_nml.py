#!/usr/bin/env python3
"""
Pure NEURON (Python) single-channel test for the NeuroML-exported BK
Ca-activated K channel `kBK_nml` (kBK_nml.mod: SUFFIX kBK_nml) on the full
IT2 geometry.

- Same geometry, passive leak, K reversal, and Ca concentrations as
  run_neuron_kBK_native.py.
- Only `kBK_nml` + passive leak (`pas`) mechanisms inserted.

Outputs (in IT2/compare):
- NEURON_kBK_nml.json: time (ms) and V_soma (mV)
- NEURON_kBK_nml.v.dat: time (s), voltage (V)
"""
...
def _build_it2_with_kBK_nml():
  """Full IT2 morphology with kBK_nml + pas."""
  ...
  # K reversal (ek handled internally in kBK_nml INITIAL but set here for consistency)
  for sec in [soma, Adend1, Adend2, Adend3, Bdend, axon]:
    for seg in sec:
      try:
        seg.ek = -104.0
...
def run():
  ...
  out_json = os.path.join(out_dir, "NEURON_kBK_nml.json")
  with open(out_json, "w") as f:
    json.dump({"simData": sim}, f, indent=2, sort_keys=True)
    f.write("\n")
  print(f"[NEURON] kBK_nml single-channel simulation -> {out_json}")

  t_sec = [float(t) / 1000.0 for t in sim["t"]]
  v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
  out_dat = os.path.join(out_dir, "NEURON_kBK_nml.v.dat")
