...
- Native A-type K channel `kap` (kap_BS.mod) + pas
  -> compare/NEURON_kap_native.json
- NeuroML-exported `kap_nml` (kap_nml.mod) + pas
  -> compare/NEURON_kap_nml.json

Both use the same IT2 geometry, K reversal, and IClamp.
"""
...
def _plot_overlay_and_diff(x_ms, y_native, y_nml, title, ylabel, out_dir: Path, fname: str):
    ...
    ax1.plot(x_ms, y_native, "b-", lw=1.4, label="kap native")
    ax1.plot(x_ms, y_nml, "m--", lw=1.2, label="kap_nml")
    ax1.set_title(title)
...
def compare(native_json: Path, nml_json: Path, out_dir: Path, title: str, out_png_name: str):
    print("=" * 80)
    print("Comparing kap: native vs kap_nml (full IT2 geometry)")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare native kap vs kap_nml full-geometry NEURON simulations."
    )
    parser.add_argument(
        "--native-json",
        type=str,
        default=str(BASE_DIR / "compare" / "NEURON_kap_native.json"),
        help="Path to native kap JSON (default: IT2/compare/NEURON_kap_native.json)",
    )
    parser.add_argument(
        "--nml-json",
        type=str,
        default=str(BASE_DIR / "compare" / "NEURON_kap_nml.json"),
        help="Path to NML-exported kap_nml JSON (default: IT2/compare/NEURON_kap_nml.json)",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default=str(SCRIPT_DIR / "kap_native_vs_nml"),
        help="Output directory for comparison plots/statistics.",
    )
    parser.add_argument(
        "--title",
        type=str,
        default="Membrane voltage: kap (native vs kap_nml)",
        help="Title for the voltage comparison plot.",
    )
    parser.add_argument(
        "--out-png-name",
        type=str,
        default="voltage_overlay_diff_kap_native_vs_nml.png",
