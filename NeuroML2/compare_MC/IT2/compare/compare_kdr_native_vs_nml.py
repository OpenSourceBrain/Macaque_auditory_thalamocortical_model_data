...
- Native delayed rectifier K channel `kdr` (kdr_BS.mod) + pas
  -> compare/NEURON_kdr_native.json
- NeuroML-exported `kdr_nml` (kdr_nml.mod) + pas
  -> compare/NEURON_kdr_nml.json
...
def _plot_overlay_and_diff(x_ms, y_native, y_nml, title, ylabel, out_dir: Path, fname: str):
    ...
    ax1.plot(x_ms, y_native, "b-", lw=1.4, label="kdr native")
    ax1.plot(x_ms, y_nml, "m--", lw=1.2, label="kdr_nml")
    ax1.set_title(title)
...
def compare(native_json: Path, nml_json: Path, out_dir: Path, title: str, out_png_name: str):
    print("=" * 80)
    print("Comparing kdr: native vs kdr_nml (full IT2 geometry)")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare native kdr vs kdr_nml full-geometry NEURON simulations."
    )
    parser.add_argument(
        "--native-json",
        type=str,
        default=str(BASE_DIR / "compare" / "NEURON_kdr_native.json"),
        help="Path to native kdr JSON (default: IT2/compare/NEURON_kdr_native.json)",
    )
    parser.add_argument(
        "--nml-json",
        type=str,
        default=str(BASE_DIR / "compare" / "NEURON_kdr_nml.json"),
        help="Path to NML-exported kdr_nml JSON (default: IT2/compare/NEURON_kdr_nml.json)",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default=str(SCRIPT_DIR / "kdr_native_vs_nml"),
        help="Output directory for comparison plots/statistics.",
    )
    parser.add_argument(
        "--title",
        type=str,
        default="Membrane voltage: kdr (native vs kdr_nml)",
        help="Title for the voltage comparison plot.",
    )
    parser.add_argument(
        "--out-png-name",
        type=str,
        default="voltage_overlay_diff_kdr_native_vs_nml.png",
