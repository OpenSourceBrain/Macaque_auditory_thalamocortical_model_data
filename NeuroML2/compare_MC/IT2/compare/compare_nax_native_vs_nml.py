#!/usr/bin/env python3
"""
Compare pure NEURON single-compartment simulations using:

- Native nax mechanism (nax_BS.mod, run_neuron_nax_native.py)
  -> NEURON_nax_native.json
- NeuroML-exported nax_nml mechanism (nax_nml.mod, run_neuron_nax_nml.py)
  -> NEURON_nax_nml.json

Both JSON files are expected to have:

  {"simData": {"t": [...], "V_soma": [...], ...}}

Traces are aligned by integer step index. Time vectors should be identical
up to floating-point noise (dt = 0.01 ms), but we check and report any
difference.
"""
...
def _plot_overlay_and_diff(x_ms, y_native, y_nml, title, ylabel, out_dir: Path, fname: str):
    ...
    ax1.plot(x_ms, y_native, "b-", lw=1.4, label="nax native")
    ax1.plot(x_ms, y_nml, "m--", lw=1.2, label="nax_nml")
    ax1.set_title(title)
...
def compare(native_json: Path, nml_json: Path, out_dir: Path, title: str, out_png_name: str):
    print("=" * 80)
    print("Comparing nax single-channel: native vs nax_nml")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare native nax vs nax_nml single-channel NEURON simulations."
    )
    parser.add_argument(
        "--native-json",
        type=str,
        default=str(BASE_DIR / "compare" / "NEURON_nax_native.json"),
        help="Path to native nax JSON (default: IT2/compare/NEURON_nax_native.json)",
    )
    parser.add_argument(
        "--nml-json",
        type=str,
        default=str(BASE_DIR / "compare" / "NEURON_nax_nml.json"),
        help="Path to NML-exported nax_nml JSON (default: IT2/compare/NEURON_nax_nml.json)",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default=str(SCRIPT_DIR / "nax_native_vs_nml"),
        help="Output directory for comparison plots/statistics.",
    )
    parser.add_argument(
        "--title",
        type=str,
        default="Membrane voltage: nax (native vs nax_nml)",
        help="Title for the voltage comparison plot.",
    )
    parser.add_argument(
        "--out-png-name",
        type=str,
        default="voltage_overlay_diff_nax_native_vs_nml.png",
