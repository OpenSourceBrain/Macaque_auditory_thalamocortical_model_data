#!/usr/bin/env python3
"""
Pure NEURON (Python) single-channel test for the native BK Ca-activated K
current (kBK.mod: SUFFIX kBK) used in the IT2 cell.

Geometry and stimulation use the full IT2_reduced morphology:

- Multi-compartment soma + Adend1–3 + Bdend + axon.
- Only native `kBK` + passive leak (`pas`) mechanisms inserted.
- Ca concentrations are clamped to cai=5e-5 mM, cao=2.0 mM on segments
  that support ca_ion, matching other Ca-related single-channel tests.
- IClamp: 0.3 nA from 200–500 ms, dt = 0.025 ms, tstop = 700 ms.

Outputs (in IT2/compare):
- NEURON_kBK_native.json: time (ms) and V_soma (mV)
- NEURON_kBK_native.v.dat: time (s), voltage (V)
"""
...
def _build_it2_with_kBK_native():
    """Full IT2 morphology with native kBK + pas."""
    ...
    # Native kBK (kBK.mod) on soma + dendrites (as in IT2_reduced)
    ...
    # Clamp Ca concentrations where present
...
def run():
    ...
    soma = _build_it2_with_kBK_native()
    ...
    out_json = os.path.join(out_dir, "NEURON_kBK_native.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] kBK native single-channel simulation ->", out_json)

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_kBK_native.v.dat")
