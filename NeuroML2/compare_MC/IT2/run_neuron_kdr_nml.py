#!/usr/bin/env python3
"""
Pure NEURON (Python) single-channel test for the NeuroML-exported delayed
rectifier K channels `kdr_nml` / `kdr_soma_nml` on the full IT2 geometry.

- Same geometry, passive leak, K reversal, and IClamp as
  run_neuron_kdr_native.py.
- Somatic group uses `kdr_soma_nml`; dendrites + axon use `kdr_nml`,
  matching IT2_reduced_cell.nml channelDensity entries.

Outputs (in IT2/compare):
- NEURON_kdr_nml.json: time (ms) and V_soma (mV)
- NEURON_kdr_nml.v.dat: time (s), voltage (V)
"""
...
def _build_it2_with_kdr_nml():
    """Full IT2 morphology with kdr_soma_nml (soma) + kdr_nml (dendrites/axon) + pas."""
    ...
    # Somatic delayed rectifier K: kdr_soma_nml (matches kdr_soma in NML)
    soma.insert("kdr_soma_nml")
    for seg in soma:
        seg.kdr_soma_nml.gmax = 0.017

    # Dendritic + axonal delayed rectifier K: kdr_nml (matches kdr in NML)
    for sec in [Adend1, Adend2, Adend3, Bdend, axon]:
        sec.insert("kdr_nml")
        for seg in sec:
            seg.kdr_nml.gmax = 0.00833766634808 if sec is not axon else 0.0416883317404

    # K reversal (ek handled internally in kdr_nml INITIAL but set here for consistency)
...
def run():
    ...
    out_json = os.path.join(out_dir, "NEURON_kdr_nml.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] kdr_nml full-geometry simulation ->", out_json)

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_kdr_nml.v.dat")
