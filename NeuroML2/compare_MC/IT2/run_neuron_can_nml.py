#!/usr/bin/env python3
"""
Pure NEURON (Python) test for NeuroML-exported N-type Ca channel `can_nml`
on the full IT2 geometry, with only `can_nml` + passive leak.

Same geometry, Ca concentrations, and stimulation as run_neuron_can_native.py.

Outputs (in IT2/compare):
- NEURON_can_nml.json
- NEURON_can_nml.v.dat
"""
...
def _build_it2_with_can_nml():
    """Full IT2 morphology with can_nml + pas."""
    ...
    # can_nml on can_group (soma + all dendrites)
...
def run():
    ...
    out_json = os.path.join(out_dir, "NEURON_can_nml.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] can_nml full-geometry simulation -> {out_json}")

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_can_nml.v.dat")
