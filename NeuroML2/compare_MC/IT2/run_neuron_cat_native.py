#!/usr/bin/env python3
"""
Pure NEURON (Python) test for native T-type Ca channel `cat` (cat_mig.mod)
on the full IT2 geometry, with only `cat` + passive leak.
...
- NEURON_cat_native.json
- NEURON_cat_native.v.dat
"""
...
def _build_it2_with_cat_native():
    """Full IT2 morphology with native cat + pas."""
    ...
    # Native T-type Ca (cat_mig.mod: SUFFIX cat), on cat_group (soma + all dendrites)
...
def run():
    ...
    soma = _build_it2_with_cat_native()
    ...
    out_json = os.path.join(out_dir, "NEURON_cat_native.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] cat native full-geometry simulation -> {out_json}")

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_cat_native.v.dat")
