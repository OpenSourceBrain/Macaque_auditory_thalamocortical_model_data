#!/usr/bin/env python3
"""
Pure NEURON (Python) single-channel test for the NeuroML-exported Ih current
`ih_nml` (ih_nml.mod: SUFFIX ih_nml) on the full IT2 geometry.

Geometry, passive leak, and stimulation match run_neuron_ih_native.py:

- Multi-compartment soma + Adend1–3 + Bdend + axon.
- Only `ih_nml` + passive leak (`pas`) mechanisms inserted.
- IClamp: 0.3 nA from 200–500 ms, dt = 0.025 ms, tstop = 700 ms.

Outputs (in IT2/compare):
- NEURON_ih_nml.json: time (ms) and V_soma (mV)
- NEURON_ih_nml.v.dat: time (s), voltage (V)
"""
...
def _build_it2_with_ih_nml():
    """Full IT2 morphology with ih_nml + pas."""
    ...
    # ih_nml on ih_group (soma + all dendrites; no axon ih)
...
def run():
    ...
    out_json = os.path.join(out_dir, "NEURON_ih_nml.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] ih_nml single-channel simulation -> {out_json}")

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_ih_nml.v.dat")
