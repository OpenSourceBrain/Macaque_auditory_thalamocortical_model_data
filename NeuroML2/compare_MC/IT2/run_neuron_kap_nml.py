#!/usr/bin/env python3
"""
Pure NEURON (Python) single-channel test for the NeuroML-exported A-type K
channel `kap_nml` (kap_nml.mod: SUFFIX kap_nml) on the full IT2 geometry.

- Same geometry, passive leak, K reversal, and IClamp as run_neuron_kap_native.py.
- Only `kap_nml` + passive leak (`pas`) mechanisms inserted.

Outputs (in IT2/compare):
- NEURON_kap_nml.json: time (ms) and V_soma (mV)
- NEURON_kap_nml.v.dat: time (s), voltage (V)
"""
...
def _build_it2_with_kap_nml():
    """Full IT2 morphology with kap_nml + pas."""
    ...
    # kap_nml on kap_group (soma + dendrites)
    for sec in [soma, Adend1, Adend2, Adend3, Bdend]:
        sec.insert("kap_nml")
        for seg in sec:
            seg.kap_nml.gmax = 0.0240195239098

    # kap_nml on axonal group
    axon.insert("kap_nml")
    for seg in axon:
        seg.kap_nml.gmax = 0.120097619549

    # K reversal (ek handled internally in kap_nml INITIAL but set here for consistency)
...
def run():
    ...
    out_json = os.path.join(out_dir, "NEURON_kap_nml.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] kap_nml single-channel simulation -> {out_json}")

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_kap_nml.v.dat")
