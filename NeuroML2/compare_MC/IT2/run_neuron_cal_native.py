#!/usr/bin/env python3
"""
Pure NEURON (Python) test for native L-type Ca channel `cal` (cal_mig.mod)
on the full IT2 geometry, with only `cal` + passive leak.

- Morphology: soma + Adend1–3 + Bdend + axon (from IT2_reduced_cell.nml)
- Mechanisms: pas + cal
- Ca concentrations: cai=5e-5 mM, cao=2.0 mM on all segments
- IClamp: 0.3 nA from 200–500 ms
- dt = 0.025 ms, tstop = 700 ms

Outputs (in IT2/compare):
- NEURON_cal_native.json
- NEURON_cal_native.v.dat
"""
...
def _build_it2_with_cal_native():
  """Full IT2 morphology with native cal + pas."""
  ...
  # Native L-type Ca (cal_mig.mod: SUFFIX cal), on cal_group (soma + all dendrites)
  for sec in [soma, Adend1, Adend2, Adend3, Bdend]:
    sec.insert("cal")
    for seg in sec:
      seg.cal.gcalbar = 2.39132864454e-06

  # Set Ca concentrations (no dynamics; cai stays constant) where present
...
def run():
  ...
  soma = _build_it2_with_cal_native()
  ...
  out_json = os.path.join(out_dir, "NEURON_cal_native.json")
  with open(out_json, "w") as f:
    json.dump({"simData": sim}, f, indent=2, sort_keys=True)
    f.write("\n")
  print(f"[NEURON] cal native full-geometry simulation -> {out_json}")

  t_sec = [float(t) / 1000.0 for t in sim["t"]]
  v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
  out_dat = os.path.join(out_dir, "NEURON_cal_native.v.dat")
