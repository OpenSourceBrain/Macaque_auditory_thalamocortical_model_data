#!/usr/bin/env python3
"""
Pure NEURON (Python) single-channel test for the native Na current
(nax_BS.mod: SUFFIX nax) used in the IT2 cell.

Geometry and stimulation use the full IT2_reduced morphology:

- Multi-compartment soma + Adend1–3 + Bdend + axon.
- Only native `nax` + passive leak (`pas`) mechanisms inserted.
- IClamp: 0.3 nA from 200–500 ms, dt = 0.025 ms, tstop = 700 ms.

Outputs:
- NEURON_nax_native.json: time (ms) and V_soma (mV)
- NEURON_nax_native.v.dat: time (s), voltage (V)
"""
...
def _build_it2_with_nax_native():
    """Full IT2 morphology with native nax + pas."""
    ...
    # Native nax (nax_BS.mod) with full IT2 distribution
...
def run():
    ...
    soma = _build_it2_with_nax_native()

    # Stimulus: 0.3 nA from 200–500 ms
    ...
    out_json = os.path.join(out_dir, "NEURON_nax_native.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] nax native single-channel simulation -> {out_json}")

    # SI-unit trace
    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_nax_native.v.dat")
