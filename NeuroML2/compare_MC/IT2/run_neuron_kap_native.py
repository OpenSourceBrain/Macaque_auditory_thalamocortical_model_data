#!/usr/bin/env python3
"""
Pure NEURON (Python) single-channel test for the native A-type K current
(kap_BS.mod: SUFFIX kap) used in the IT2 cell.

Geometry and stimulation use the full IT2_reduced morphology:

- Multi-compartment soma + Adend1–3 + Bdend + axon.
- Only native `kap` + passive leak (`pas`) mechanisms inserted.
- IClamp: 0.3 nA from 200–500 ms, dt = 0.025 ms, tstop = 700 ms.

Outputs (in IT2/compare):
- NEURON_kap_native.json: time (ms) and V_soma (mV)
- NEURON_kap_native.v.dat: time (s), voltage (V)
"""
...
def _build_it2_with_kap_native():
    """Full IT2 morphology with native kap + pas."""
...
def run():
    ...
    soma = _build_it2_with_kap_native()
    ...
    out_json = os.path.join(out_dir, "NEURON_kap_native.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] kap native single-channel simulation -> {out_json}")

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_kap_native.v.dat")
