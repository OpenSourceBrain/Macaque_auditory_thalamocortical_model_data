#!/usr/bin/env python3
"""
Pure NEURON (Python) test for NeuroML-exported T-type Ca channel `cat_nml`
on the full IT2 geometry, with only `cat_nml` + passive leak.

Same geometry, Ca concentrations, and stimulation as run_neuron_cat_native.py.

Outputs (in IT2/compare):
- NEURON_cat_nml.json
- NEURON_cat_nml.v.dat
"""
...
def _build_it2_with_cat_nml():
    """Full IT2 morphology with cat_nml + pas."""
    ...
    # cat_nml on cat_group (soma + all dendrites)
...
def run():
    ...
    out_json = os.path.join(out_dir, "NEURON_cat_nml.json")
    with open(out_json, "w") as f:
        json.dump({"simData": sim}, f, indent=2, sort_keys=True)
        f.write("\n")
    print(f"[NEURON] cat_nml full-geometry simulation -> {out_json}")

    t_sec = [float(t) / 1000.0 for t in sim["t"]]
    v_v = [float(v) / 1000.0 for v in sim["V_soma"]]
    out_dat = os.path.join(out_dir, "NEURON_cat_nml.v.dat")
