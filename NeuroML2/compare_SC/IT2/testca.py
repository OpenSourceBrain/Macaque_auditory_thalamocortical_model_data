...
def test_channel_nml(
    ...
    newdoc = read_neuroml2_file("testcell.nml")
    newnet = newdoc.add(
        neuroml.Network,
        id="testnet",
    ...
    newpop = newnet.add(neuroml.Population, id="testpop", component="testcell", size=1)
    ...
    if channel != "pas":
        ...
        for gate in gates:
            ...
            recorder_dict[f"{timestamp}_{gate}_nml.dat"].append(
                f"{newpop.id}[0]/biophys/membraneProperties/cat_dend_soma/{channel}/{gate}/q"
            )
    if ca is True:
        recorder_dict[f"{timestamp}_ca_nml.dat"] = [f"{newpop.id}[0]/caConc"]
        recorder_dict[f"{timestamp}_ca_i_nml.dat"] = [f"{newpop.id}[0]/biophys/membraneProperties/cat_dend_soma/iDensity"]
    ...
    if ca is True:
        ...
        recorded_ca_i = numpy.array(data.get(f"{newpop.id}[0]/biophys/membraneProperties/cat_dend_soma/iDensity"))
